 ...
 for i = 1:k
   ...
   constraint=0;
    ...
    groupsalltrain = Atest(:,3487);
     ...
     [B,FitInfo] = lasso(Anew,groupsalltrain,'NumLambda',75,'CV',10,'alpha', alpha);
     indx = FitInfo.IndexMinMSE;
     B0 = B(:,indx);
     predictors = find(B0);
     
     if isempty(predictors) ==1
         continue;
     end;
    for l= 1:length(predictors)
         countFC(j,predictors(l)) = countFC(j,predictors(l))+1;
     end;
         
     
     lambdaindex = FitInfo.IndexMinMSE;
%       FitInfo.MSE(lambdaindex)
 newpredictor = predictors';
...
if length(C_final)>1 
 ...
 constraint
    ...
    svmModel = fitcsvm(newA1(trainIdxnew,:), groupsall(trainIdxnew), ...
                 'Standardize',true,'KernelFunction','linear','BoxConstraint',constraint ,'KernelOffset',0);
